...
const Quantity = ({ value = 0, onIncrease, onDecrease, disabled = false }) => {
  ...
  return (
    ...
    <div className="flex items-center justify-center space-x-3 mt-auto pt-2">
      ...
      >
        <span>−</span>{" "}
        {/* Usamos el símbolo matemático 'minus' real, no el guion */}
      </button>
      ...
      <button
        ...
        className={`w-10 h-10 flex items-center justify-center rounded-lg font-extrabold text-xl
                    ...
                    focus:outline-none focus:ring-2 focus:ring-yellow-400
                    ${
                      // 🚨 ESTILO CONDICIONAL: Cambia a gris si 'disabled' (isMaxed) es true
                      disabled
                      ? "bg-gray-200 text-gray-400 cursor-not-allowed" // Estilo Deshabilitado (Gris)
                      : "bg-red-700 text-yellow-400 hover:bg-red-600" // Estilo Normal
                    }`}
      >
        <span>+</span>
      </button>
