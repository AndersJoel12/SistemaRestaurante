...
const TablesGrid = ({
  ...
  const [errorMsg, setErrorMsg] = useState(""); // 🔥 currentSelectedTable ahora es simplemente el prop selectedTable

  const currentSelectedTable = selectedTable; // --- CARGAR MESAS ---
  ...
  return (
    <div className="p-4 sm:p-6 bg-gray-100 min-h-screen flex flex-col items-center">
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-4xl border border-gray-200">
        ...
                {/* GRID RESPONSIVE: Usa más columnas en pantallas grandes */} 
             {" "}
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 animate-fade-in-up">
            ...
            {currentSelectedTable.status === "libre" ? (
              <div className="bg-green-50 p-6 rounded-xl border border-green-200 shadow-inner">
                               {" "}
                {/* Contenedor principal de info y input, ahora flexible en SM */}
                               {" "}
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                                    {/* Info de la mesa */}                 {" "}
                  <div>
                                       {" "}
                    <h3 className="text-xl font-bold text-green-800 text-center sm:text-left">
                                            Mesa {currentSelectedTable.number}{" "}
                      Disponible                    {" "}
                    </h3>
                                       {" "}
                    <p className="text-green-600 text-sm text-center sm:text-left">
                                            Capacidad:{" "}
                      {currentSelectedTable.capacity} personas                  
                       {" "}
                    </p>
                                     {" "}
                  </div>
...
                      `}
                ...
                               {" "}
                {/* 🔥 Contenedor de botones de acción para MESA LIBRE (Flex en sm, Columna en default) */}
                               {" "}
                <div className="flex flex-col sm:flex-row gap-4 mt-4">
                                   {" "}
                  {/* Botón 1: Confirmar Mesa (Apartar) - w-full siempre */}   
                               {" "}
                  <button
...
                    `}
                  ...
                                   {" "}
                  {/* Botón 2: CANCELAR (Inferior) - w-full siempre, pero mantiene tamaño fijo en sm */}
                                   {" "}
                  <button
                    onClick={() => onCancelAction()}
                    className="w-full sm:w-auto py-3 px-6 bg-gray-500 text-white font-bold rounded-xl shadow-lg hover:bg-gray-600 transition transform active:scale-95 flex-shrink-0"
              ...
              <div className="bg-red-50 p-6 rounded-xl border border-red-200 shadow-inner text-center">
                ...
                               {" "}
                {/* 🔥 Contenedor de botones de acción para MESA OCUPADA (Flex en sm, Columna en default) */}
                               {" "}
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                   {" "}
                  {/* Botón 1: Ver / Editar Pedido - w-full siempre, sm:w-auto en desktop */}
                                   {" "}
                  <button
                  ...
                                   {" "}
                  {/* Botón 2: CANCELAR (Inferior) - w-full siempre, sm:w-auto en desktop */}
                                   {" "}
                  <button
                    onClick={() => onCancelAction()}
