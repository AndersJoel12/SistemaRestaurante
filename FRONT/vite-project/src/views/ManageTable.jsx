...
      
      // Buscamos por número o ubicación
      const numStr = String(mesa.numero);
      const ubicacionStr = (mesa.ubicacion || "").toLowerCase();
      
      const matchesSearch = numStr.includes(term) || ubicacionStr.includes(term);
...
    try {
        // Validaciones
        if (!editingItem.numero || !editingItem.capacidad || !String(editingItem.ubicacion).trim()) {
...
            setMesas(prev => prev.map(m => m.id === editingItem.id ? response.data : m));
...
    } catch (error) {
      setMessage({ type: "error", text: "Error al eliminar (asegúrate de que no tenga pedidos)." });
    } finally {
      setLoading(false);
    }
  };
...
          <h1 className="text-2xl md:text-4xl font-extrabold text-yellow-400">Gestión de Mesas</h1>
...
                <div className={`absolute top-3 right-3 w-3 h-3 rounded-full ${mesa.estado ? "bg-green-500 animate-pulse" : "bg-red-500"}`}></div>

                {/* Número de Mesa Gigante */}
                <div className={`text-4xl font-extrabold mb-1 ${mesa.estado ? "text-gray-800" : "text-gray-400"}`}>
                  {mesa.numero}
                </div>

                {/* Detalles */}
                <div className="text-center">
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-wider truncate max-w-[8rem]">{mesa.ubicacion}</p>
...
            
            <div className="bg-red-800 p-4 text-white flex justify-between items-center">
              <h2 className="text-xl font-bold">{editingItem.id ? `Editar Mesa ${editingItem.numero}` : "Nueva Mesa"}</h2>
