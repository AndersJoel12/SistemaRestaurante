...
      setMessage({ type: "error", text: "No se pudo cargar la lista de categorías." });
    } finally {
      setLoading(false);
    }
  }, []);
...
            
            setCategorias(prev => prev.map(c => c.id === editingItem.id ? response.data : c));
...
      setMessage({ type: "error", text: "No se puede eliminar (quizás tiene productos asociados)." });
    } finally {
      setLoading(false);
    }
  };
...
  return (
    <div className="bg-gray-100 min-h-screen font-sans pb-24"> {/* Padding bottom para NavBar fijo */}
      
      {/* Contenedor Principal con padding adaptativo */}
      <div className="p-4 md:p-6 max-w-7xl mx-auto">
      
        {/* HEADER */}
        <div className="bg-red-800 text-white p-4 md:p-6 rounded-xl shadow-xl mb-6 flex justify-between items-center">
          <h1 className="text-2xl md:text-4xl font-extrabold text-yellow-400">Gestión de Categorías</h1>
...
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:hidden">
            {filteredCategorias.length === 0 ? (
                <div className="text-center p-8 bg-white rounded-xl text-gray-500 col-span-full shadow-md">No se encontraron categorías.</div>
            ) : (
                filteredCategorias.map((cat) => {
                    const statusClass = cat.estado ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700";
                    return (
                        <div key={cat.id} className="bg-white p-4 rounded-xl shadow-md flex flex-col gap-3 border border-gray-100">
                            ...
                            <div className="flex justify-end gap-3 pt-3 border-t">
                                <button onClick={() => openModal(cat)} className="px-3 py-1.5 text-sm bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-colors">
                                    ✏️ Editar
                                </button>
                                <button onClick={() => handleDelete(cat.id)} className="px-3 py-1.5 text-sm bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors">
                                    🗑️ Eliminar
                                </button>
                            </div>
                        </div>
...
              {filteredCategorias.length === 0 ? (
                <tr><td colSpan="5" className="text-center py-8 text-gray-500">No se encontraron categorías.</td></tr>
              ) : (
                filteredCategorias.map((cat) => (
...
            
            <div className="bg-red-800 p-4 text-white flex justify-between items-center">
              <h2 className="text-xl font-bold">{editingItem.id ? "Editar Categoría" : "Crear Categoría"}</h2>
