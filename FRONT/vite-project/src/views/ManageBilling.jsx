...
const GestionFacturacion = () => {
  ...
  const handleImprimir = (factura) => {
    ...
    ventanaImpresion.document.write(`
        ...
        <body>
            ...
            <div class="totals">
            <div>Subtotal: $${parseFloat(factura.subtotal || 0).toFixed(
              2
            )}</div>
            <div>Impuesto: $${(
              parseFloat(factura.totalFactura) -
              parseFloat(factura.subtotal) +
              parseFloat(factura.descuento || 0)
            ).toFixed(2)}</div>
            <div>Descuento: -$${parseFloat(factura.descuento || 0).toFixed(
              2
            )}</div>
            <div class="total-row">TOTAL: $${parseFloat(
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: "Error al actualizar. Verifica permisos.",
      });
    } finally {
      setLoading(false);
    }
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4">
                <div className="w-2/3">
                  ...
                  <input
                    type="text"
                    value={editingItem.cliente_nombre}
                    disabled
                    className="w-full bg-gray-100 border p-2 rounded text-gray-600"
                  />
                </div>
                <div className="w-1/3">
                  <label className="block text-xs font-bold text-gray-500">
                ...
                <div className="flex gap-4 mt-3">
                  <div className="w-1/2">
                    ...
                    <select
                      ...
                      className="w-full border p-2 rounded-md focus:ring-red-500"
                    >
                      {METODOS_PAGO.map((m) => (
            ...
            {filteredFacturas.length === 0 ? (
              <tr>
                <td colSpan="7" className="text-center py-8 text-gray-500">
                  No hay facturas registradas o encontradas.
                </td>
              </tr>
            ) : (
