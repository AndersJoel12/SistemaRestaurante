...
const Kitchen = () => {
  ...
  const retrocederEstado = async (orden) => {
    let nuevoEstado = "";
    switch (orden.estado_pedido) {
      case ESTADOS.COCINANDO: nuevoEstado = ESTADOS.NUEVO; break; // De Fuego a Nuevo
      case ESTADOS.LISTO: nuevoEstado = ESTADOS.COCINANDO; break; // De Listo a Fuego
      default: return;
    }
    actualizarEstado(orden, nuevoEstado);
  };
      ...
      <>
        <ul className="ml-4 list-disc text-sm text-gray-700 mb-3 space-y-1">
          {items.map((item, idx) => {
            ...
            return <li key={item.id || idx}><span className="font-bold">{item.cantidad || 1}x</span> {nombre}</li>;
          })}
        </ul>
        {observacion && <div className="bg-white p-2 rounded text-xs text-red-600 border border-red-100 mb-3 italic font-bold">📝 {observacion}</div>}
      </>
    );
  };
